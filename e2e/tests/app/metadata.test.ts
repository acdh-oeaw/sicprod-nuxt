...
test.describe("should set page metadata", () => {
	...
	test.skip("with en locale", async ({ page }) => {
		...
		await expect(metaDescription).toHaveAttribute("content", "SiCProD");

		const ogTitle = page.locator('meta[property="og:title"]');
		await expect(ogTitle).toHaveAttribute("content", "Home");

		const ogDescription = page.locator('meta[property="og:description"]');
		await expect(ogDescription).toHaveAttribute("content", "SiCProD");
	...
	test("with de locale", async ({ page }) => {
		...
		await expect(page).toHaveTitle("Startseite | SiCProD");
